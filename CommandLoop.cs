...
namespace SampleClientApp
{
    public class CommandLoop
    {
        ...
        public async Task CommandCreateDigitalTwin(string[] cmd)
        {
            ...
            {
                if (args[i + 1] != "metadata")
                    twinData.Contents.Add(args[i], ConvertStringToType(args[i + 1], args[i + 2]));
                else
                {
                    var component = new BasicDigitalTwinComponent();
                    string componentArgsString = args[i + 2];
                    string[] componentArgs = componentArgsString.Split(',');
                    if (componentArgs.Length > 1) {
                        for (int j = 0; j < componentArgs.Length; j += 3)
                        {
                            component.Contents.Add(componentArgs[j], ConvertStringToType(componentArgs[j + 1], componentArgs[j + 2]));
                        }
                    }
                    twinData.Contents.Add(args[i], component);
            ...
            try
            {
                Console.WriteLine(Newtonsoft.Json.JsonConvert.SerializeObject(twinData));
